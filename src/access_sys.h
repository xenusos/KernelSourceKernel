/*
	...
	Purpose: hacky and ugly header that gets around the limitation of no IAT while including common apis that xlib that don't require initialization. 
*/
#pragma once
...
#include <libx/xenus_strings.h>  // DOES NOT DEPEND ON A CALL TO XLIB_START!
								 // this is just formatting stuff


static size_t string_format(char * buffer, size_t size, const char *fmt, ...)
{
	va_list ap;
	size_t ret;

	va_start(ap, fmt);
	ret = vsnprintf(buffer, size, fmt, ap); //TODO check response
	va_end(ap);
